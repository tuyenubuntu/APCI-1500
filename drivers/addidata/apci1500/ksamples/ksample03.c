...
*
* Demonstrate how to use the driver in kernel mode with APCI-1500 
* and timer 2 interrupt. When the timer 2 interrupt occured,
* read digital input status and set digital output.
*/
...
#define MODULE_NAME "ksample03 apci1500"
   		...
   		if 	(0x7ffffffe != info[0])
   			{   	
		   		...
		   		if (info[0] & 0x8) {	   
		   			...
		   			/* Read status of the 16 digital input */
		   			i_APCI1500_Read16DigitalInput (pdev, &status);						
		   			printk (KERN_DEBUG "Timer 2 interrupt, digital input status = %x\n", status);
		   			
		   			/* Set the digital output */
		   			i_APCI1500_Set16DigitalOutputOn(pdev, (1 << counter++));
		   			
		   			if (counter == 9) {
						i_APCI1500_SetOutputMemoryOn (pdev);	
						
						/* When the output memory is on the actual status 
						 * of the digital output is holding and the new status 
						 * is added to the old. To reset an output use 
						 * i_APCI1500_SetXDigitalOutputOff.
						 */  				
		   			}
		   			
		   			if (counter > 15) {
		   				counter = 0;
						i_APCI1500_SetOutputMemoryOff (pdev);		   				
		   			}
	   			}	   			
  			}
...
static int ksample03_init(struct pci_dev * pdev)
{
	int error = 0;
	unsigned long arg = 0;
	...
	error = i_APCI1500_SetOutputMemoryOn (pdev);	
	
	printk (KERN_DEBUG "i_APCI1500_SetOutputMemoryOn");   
	switch (error) {
		case 0: printk (KERN_DEBUG "OK\n");
		break;
	}
	...
	arg = APCI1500_ENABLE;
	error = i_APCI1500_EnableDisableTimerCounter2Interrupt(pdev, arg);
	
	printk (KERN_DEBUG "i_APCI1500_EnableDisableTimerCounter2Interrupt:");   
	switch (error) {
		case 0: printk (KERN_DEBUG "OK\n");
		break;
	...
	
	{
		...
		short us_ReloadValue = 437; // 0,5 seconde
		unsigned long argInit = MAKELONG( us_TimerMode, us_ReloadValue );	
		
		error = i_APCI1500_InitTimerCounter2(pdev, argInit);
		
		printk (KERN_DEBUG "i_APCI1500_InitTimerCounter2:");   
		switch (error) {
			case 0: printk (KERN_DEBUG "OK\n");
			break;
		}
	...
	error = i_APCI1500_StartTimerCounter2(pdev);
	
	printk (KERN_DEBUG "i_APCI1500_StartTimerCounter2:");   
	switch (error) {
		case 0: printk (KERN_DEBUG "OK\n");
		break;
		case -EINVAL: printk (KERN_DEBUG "TimerCounter2 not initialised\n");
		break;			
	}	
...
static void ksample03_release(struct pci_dev * pdev)
{				
	int error = 0;
	unsigned long arg = 0;
		
	error = i_APCI1500_StopTimerCounter2(pdev);
	
	printk (KERN_DEBUG "i_APCI1500_StopTimerCounter2");   
	switch (error) {
		case 0: printk (KERN_DEBUG "OK\n");
		break;
	}
	...
	/* Release the output memory */
	error = i_APCI1500_SetOutputMemoryOff (pdev);	
	
	printk (KERN_DEBUG "i_APCI1500_SetOutputMemoryOff");   
	switch (error) {
		case 0: printk (KERN_DEBUG "OK\n");
		break;
	}
...
{	
	...
	for (index=0; index<NUMBER_OF_BOARDS; index++)
	{
		...
		{
			if ( ksample03_init (pdev) )
			{
				/* Reset the user interrupt callback */
				ksample03_release (pdev);
