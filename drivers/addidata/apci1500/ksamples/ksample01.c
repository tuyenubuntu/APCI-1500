...
*
* Demonstrate how to use the driver in kernel mode with APCI-1500 and input interrupt event.
*/
...
#define MODULE_NAME "ksample01 apci1500"
...
static int ksample01_init(struct pci_dev * pdev)
{
	int error = 0;
	unsigned long arg = 0;
	...
	arg = MAKE_APCI1500_INPUT_EVENT_ARG_PORT1 (	APCI1500_INPUT_PORT_1, 
													APCI1500_OR, 
													"11111111");
													
	error = i_APCI1500_SetInputEventMask(pdev, arg);	
                                            		
	printk (KERN_DEBUG "i_APCI1500_SetInputEventMask on port 1:");   
	switch (error) {
		case 0: printk (KERN_DEBUG "OK\n");
		break;
	...
	/* For the mask configuration, please see in apci1500.h */						
	arg = MAKE_APCI1500_INPUT_EVENT_ARG_PORT2 (	APCI1500_INPUT_PORT_2, 
													APCI1500_OR, 
													"111111");
													
	error = i_APCI1500_SetInputEventMask(pdev, arg);	
                                            		
	printk (KERN_DEBUG "i_APCI1500_SetInputEventMask on port 2:");   
	switch (error) {
		case 0: printk (KERN_DEBUG "OK\n");
		break;
		case -EINVAL: printk (KERN_DEBUG "arg value is wrong\n");
	...
	error = i_APCI1500_StartInputEvent(pdev, (APCI1500_INPUT_PORT_1 | APCI1500_INPUT_PORT_2));
	
	printk (KERN_DEBUG "i_APCI1500_StartInputEvent");   
	switch (error) {
		case 0: printk (KERN_DEBUG "OK\n");
		break;
		case -EINVAL: printk (KERN_DEBUG "arg value is wrong\n");
...
static void ksample01_release(struct pci_dev * pdev)
{				
	int error = 0;
		
	error = i_APCI1500_StopInputEvent(pdev, (APCI1500_INPUT_PORT_1 | APCI1500_INPUT_PORT_2));
	
	printk (KERN_DEBUG "i_APCI1500_StopInputEvent");   
	switch (error) {
		case 0: printk (KERN_DEBUG "OK\n");
		break;
		case -EINVAL: printk (KERN_DEBUG "arg value is wrong\n");
...
{	
	...
	for (index=0; index<NUMBER_OF_BOARDS; index++)
	{
		...
		{
			if ( ksample01_init (pdev) )
			{
				/* Reset the user interrupt callback */
				ksample01_release (pdev);
