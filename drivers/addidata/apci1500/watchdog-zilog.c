...
#define MSG_UNAVAILABLE printk(KERN_WARNING "%s: this function is not available for this board version\n",__DRIVER_NAME)

//------------------------------------------------------------------------------
int do_CMD_APCI1500_InitWatchdogCounter3(struct pci_dev * pdev , unsigned int cmd, unsigned long arg)
{
...
//------------------------------------------------------------------------------
int do_CMD_APCI1500_TriggerWatchdog(struct pci_dev * pdev, unsigned int cmd, unsigned long arg )
{
	unsigned long irqstate;
	int ret;
	APCI1500_LOCK(pdev,&irqstate);
...
//------------------------------------------------------------------------------
int do_CMD_APCI1500_TriggerCounter3(struct pci_dev * pdev, unsigned int cmd, unsigned long arg )
{
	unsigned long irqstate;
	int ret;
	APCI1500_LOCK(pdev,&irqstate);
	{	
		ret = i_APCI1500_TriggerCounter3(pdev);
	}
	APCI1500_UNLOCK(pdev,irqstate);
...
//------------------------------------------------------------------------------
int do_CMD_APCI1500_StopCounter3(struct pci_dev * pdev, unsigned int cmd, unsigned long arg )
{
	unsigned long irqstate;
	int ret;
  	APCI1500_LOCK(pdev,&irqstate);
...
int do_CMD_APCI1500_EnableDisableWatchdogCounter3Interrupt(struct pci_dev * pdev, unsigned int cmd, unsigned long arg)
{
	...
	int ret;
  	APCI1500_LOCK(pdev,&irqstate);
	{
		ret =i_APCI1500_EnableDisableWatchdogCounter3Interrupt(pdev, arg);
	}
	APCI1500_UNLOCK(pdev,irqstate);
...
int do_CMD_APCI1500_StartCounter3(struct pci_dev * pdev, unsigned int cmd, unsigned long arg )
{
	unsigned long irqstate;
	int ret;
	APCI1500_LOCK(pdev,&irqstate);
...
do_CMD_APCI1500_ReadCounter3(struct pci_dev* pdev, unsigned int cmd, unsigned long arg)
{
	...
	APCI1500_LOCK(pdev,&irqstate);
	{
		ret = i_APCI1500_ReadCounter3(pdev, &l_ReadValue);
	}
	APCI1500_UNLOCK(pdev,irqstate);
